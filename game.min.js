...
(function(window){"use strict";Number.prototype.map=function(istart,istop,ostart,ostop){return ostart+(ostop-ostart)*((this-istart)/(istop-istart));};Number.prototype.limit=function(min,max){return Math.min(max,Math.max(min,this));};Number.prototype.round=function(precision){precision=Math.pow(10,precision||0);return Math.round(this*precision)/precision;};Number.prototype.floor=function(){return Math.floor(this);};Number.prototype.ceil=function(){return Math.ceil(this);};Number.prototype.toInt=function(){return(this|0);};Number.prototype.toRad=function(){return(this/180)*Math.PI;};Number.prototype.toDeg=function(){return(this*180)/Math.PI;};String.prototype.toNumber=function(){return parseFloat(this.replace(/[^\d.-]/g,""))||0;}
Array.prototype.erase=function(item){for(var i=this.length;i--;){if(this[i]===item){this.splice(i,1);}}
return this;};Array.prototype.random=function(){return this[Math.floor(Math.random()*this.length)];};Array.prototype.findId=function(id){if(!id)return false;else var r=-1,id=String(id).toNumber();for(let i=0;i<this.length;i++){if(this[i]instanceof Object&&typeof this[i]["id"]=="number"&&this[i]["id"]==id)r=i;}
return r==-1?false:this[r];}
Math.normalise=function(val,min,max){return(val-min)/(max-min);}
Math.lerp=function(n,i,x){return(x-i)*n+i;}
Math.map=function(v,i,x,j,y){return lerp(Math.normalise(v,i,x),j,y);}
Math.clamp=function(val,min,max){return Math.min(Math.max(val,min),max);}
Math.randomFloat=function(min,max){return min+Math.random()*(max-min);}
Math.randomInt=function(min,max){return Math.floor(Math.randomFloat(min,max+1));}
...
ig.baked=true;ig.module("game.entities.player").requires("impact.entity").defines(function(){Player=ig.Entity.extend({size:{x:50,y:30},pos:{x:0,y:0},vel:{x:0,y:0},maxVel:{x:1000,y:0},type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.B,cooldown:0,init:function(x,y,opt){this.parent();this.pos.x=(ig.game.width()/2)-(this.size.x/2);this.pos.y=ig.game.height()-40;},update:function(){this.parent();this.vel.x=ig.input.state("A")&&ig.game.playing?-350:ig.input.state("D")&&ig.game.playing?350:0;this.pos.x=Math.clamp(this.pos.x,15,ig.game.width()-this.size.x-15);this.cooldown=Math.clamp(this.cooldown-=ig.system.tick,0,10);if(ig.input.pressed("F")&&this.cooldown==0&&ig.game.bullets>0&&ig.game.playing){this.cooldown+=0.1;ig.game.bullets--;var spawnLocX=this.pos.x+(this.size.x/2)-2.5,bulletProps={type:ig.Entity.TYPE.A,speed:-1000};ig.game.spawnEntity(Bullet,spawnLocX,this.pos.y,bulletProps);}},draw:function(){var ctx=ig.system.context;ctx.beginPath();ctx.fillStyle="#09B500";ctx.fillRect(0,ig.game.height()-30,ig.game.width(),30);ctx.stroke();ctx.beginPath();ctx.fillStyle="#FFF";ctx.fillRect(this.pos.x,this.pos.y,this.size.x,this.size.y);ctx.stroke();this.parent();}});});

// lib/game/entities/alien.js
ig.baked=true;ig.module("game.entities.alien").requires("impact.entity").defines(function(){Aliens=ig.Entity.extend({size:{x:0,y:0},pos:{x:0,y:0},vel:{x:0,y:0},maxVel:{x:0,y:0},type:ig.Entity.TYPE.B,checkAgainst:ig.Entity.TYPE.A,aliens:[],opt:{amount:21,length:8,size:{x:30,y:30},direction:Math.random()>0.5?1:-1,between:15,moveCount:0,moveCountWait:1.5,didFall:false,difficulty:1},isDead:function(){var target=this.opt.amount,length=this.aliens.length,current=0;if(length==0)return false;for(var i=0;i<length;i++)if(this.aliens[i].dead==true)current++;if(current==target)return true;else return false;},testWin:function(){for(var i=0,l=this.aliens.length;i<l;i++)this.aliens[i].dead=true;return true;},lowestAlive:function(){for(var i=0,l=this.aliens.length,r=-1;i<l;i++)if(this.aliens[i].dead==false)r=i+1;return r;},init:function(x,y,opt){this.parent();var totalRows=Math.ceil(this.opt.amount/this.opt.length);this.size.x=(this.opt.length*this.opt.size.x)+(Math.max(this.opt.length-1,0)*this.opt.between);this.size.y=(totalRows*this.opt.size.y)+(Math.max(totalRows-1,0)*this.opt.between);this.pos.x=(ig.game.width()/2)-(this.size.x/2);this.pos.y=60;for(var i=1;i<=this.opt.amount;i++){var locCol=(function(i,l){return i!==0&&(i%l)==0?l:i%l;})(i,this.opt.length),locRow=Math.ceil(i/this.opt.length),curLen=Math.clamp(this.opt.amount-(this.opt.length*(locRow-1)),1,this.opt.length),xOffset=(this.size.x/2)-(curLen*this.opt.size.x/2)-(((curLen-1)*this.opt.size.x/2)/2);this.aliens.push({dead:false,offset:xOffset,x:xOffset+((locCol-1)*(this.opt.size.x/2))+((locCol-1)*this.opt.size.x),y:((locRow-1)*this.opt.size.y)+((locRow-1)*(this.opt.size.y/2))});}},update:function(){this.parent();if(ig.game.playing){var totalRows=Math.ceil(this.lowestAlive()/this.opt.length);this.size.y=(totalRows*this.opt.size.y)+(Math.max(totalRows-1,0)*this.opt.between);this.opt.moveCount+=ig.system.tick*this.opt.difficulty;if(this.opt.moveCount>=this.opt.moveCountWait){this.opt.moveCount-=this.opt.moveCountWait;this.pos.x+=this.opt.size.x/2*this.opt.direction;}}
var xMin=0+this.opt.size.x/2,xMax=ig.game.width()-this.size.x-this.opt.size.x/2;if(this.pos.x<xMin||this.pos.x>xMax){this.opt.direction*=-1;this.pos.x+=this.opt.size.x/2*this.opt.direction;this.pos.y+=this.opt.size.y/2;this.opt.difficulty*=1.15;}
this.pos.x=Math.clamp(this.pos.x,xMin,xMax);},check:function(entity){if(!entity)return false;if(!ig.game.playing)return false;for(var i=0,l=this.aliens.length;i<l;i++){if(this.aliens[i].dead!==true){var b1={x:entity.pos.x,y:entity.pos.y,w:entity.size.x,h:entity.size.y};var b2={x:this.pos.x+this.aliens[i].x,y:this.pos.y+this.aliens[i].y,w:this.opt.size.x,h:this.opt.size.y};if(b1.x<b2.x+b2.w&&b1.x+b1.w>b2.x&&b1.y<b2.y+b2.h&&b1.h+b1.y>b2.y){this.aliens[i].dead=true;entity.kill();ig.game.bullets++;ig.game.bulletCount=0;}}}},draw:function(){var ctx=ig.system.context;var anchor=this.pos,size=this.opt.size,aliens=this.aliens;if(ig.game.debug==true){ctx.beginPath();ctx.strokeStyle="#FF0000";ctx.moveTo(anchor.x+(this.size.x/2),anchor.y-10);ctx.lineTo(anchor.x+(this.size.x/2),anchor.y);ctx.stroke();ctx.beginPath();ctx.strokeStyle="#4AFF62";ctx.rect(anchor.x,anchor.y,this.size.x,this.size.y);ctx.stroke();}
for(var i=0,l=aliens.length;i<l;i++){if(ig.game.debug==true){ctx.beginPath();ctx.strokeStyle="#FF0000";ctx.moveTo(anchor.x+aliens[i].offset,(anchor.y+aliens[i].y)-5||0);ctx.lineTo(anchor.x+aliens[i].offset,(anchor.y+aliens[i].y)+5||0);ctx.stroke();}
if(aliens[i].dead!==true){ctx.beginPath();ctx.fillStyle="#E5FF66";ctx.fillRect((anchor.x+aliens[i].x)||0,(anchor.y+aliens[i].y)||0,size.x||0,size.y||0);ctx.stroke();}}
this.parent();}});});

// lib/game/entities/bullet.js
ig.baked=true;ig.module("game.entities.bullet").requires("impact.entity").defines(function(){Bullet=ig.Entity.extend({name:"Bullet"+parseInt(Math.random().toString().substring(2,8)),size:{x:5,y:5},pos:{x:0,y:0},vel:{x:0,y:0},maxVel:{x:0,y:1000},speed:0,init:function(x,y,opt){this.parent(x,y,opt);},update:function(){this.parent();this.vel.y=ig.game.playing?this.speed:0;if(this.pos.x<0-10-this.size.x)this.kill();if(this.pos.y<0-10-this.size.y)this.kill();if(this.pos.x>ig.game.width()+10+this.size.x)this.kill();if(this.pos.y>ig.game.height()+10+this.size.y)this.kill();},draw:function(){var ctx=ig.system.context;ctx.beginPath();ctx.fillStyle="#FFF";ctx.fillRect(this.pos.x||0,this.pos.y||0,this.size.x,this.size.y);ctx.stroke();this.parent();}});});

// lib/game/main.js
ig.baked=true;ig.module("game.main").requires("impact.game","game.entities.player","game.entities.alien","game.entities.bullet").defines(function(){MyGame=ig.Game.extend({root:window.top.location.href.split("/").reverse().splice(1).reverse().join("/")+"/",width:function(){return typeof ig.system=="undefined"?0:Math.max(ig.system.width||0,ig.system.realWidth||0)},height:function(){return typeof ig.system=="undefined"?0:Math.max(ig.system.height||0,ig.system.realHeight||0)},debug:false,bullets:5,score:0,player:null,aliens:null,playing:true,dead:false,won:false,deathCount:0,deathCountWait:5,bulletCount:0,bulletCountWait:10,reset:function(){this.playing=false;this.won=false;this.dead=false;this.deathCount=0;this.bullets=5;this.score=0;ig.game.entities.forEach(function(entity){if(entity)entity.kill()});this.player=ig.game.spawnEntity(Player);this.aliens=ig.game.spawnEntity(Aliens);this.playing=true;},init:function(){ig.input.bind(ig.KEY.A,"A");ig.input.bind(ig.KEY.LEFT_ARROW,"A");ig.input.bind(ig.KEY.D,"D");ig.input.bind(ig.KEY.RIGHT_ARROW,"D");ig.input.bind(ig.KEY.SPACE,"F");ig.input.bind(ig.KEY.ENTER,"F");ig.input.bind(ig.KEY.W,"F");ig.input.bind(ig.KEY.UP_ARROW,"F");ig.input.bind(ig.KEY.MOUSE1,"L");ig.input.bind(ig.KEY.P,"P");ig.input.bind(ig.KEY.R,"R");ig.input.bind(ig.KEY.B,"B");this.player=ig.game.spawnEntity(Player);this.aliens=ig.game.spawnEntity(Aliens);},update:function(){this.parent();this.score=Math.clamp(this.score,0,10);if(this.playing){this.bulletCount+=ig.system.tick;if(this.bulletCount>=this.bulletCountWait){this.bulletCount-=this.bulletCountWait;this.bullets++}}
this.bullets=Math.clamp(this.bullets,0,5);if(ig.input.pressed("L")){var m={x:ig.input.mouse.x,y:ig.input.mouse.y};var rb={x:this.width()-30,y:10,w:20,h:20};var pb={x:this.width()-60,y:10,w:20,h:20};if(m.x>pb.x&&m.x<pb.x+pb.w&&m.y>pb.y&&m.y<pb.y+pb.h){if(!this.dead)this.playing=!this.playing;}
if(m.x>rb.x&&m.x<rb.x+rb.w&&m.y>rb.y&&m.y<rb.y+rb.h){this.reset();}}
if(ig.input.pressed("P")){if(!this.dead)this.playing=!this.playing;}
if(ig.input.pressed("R")){this.reset();}
if(ig.input.pressed("B")){this.debug=!this.debug;}
if(this.playing){if(this.aliens.pos.y>=this.height()-80-this.aliens.size.y){this.dead=true;this.playing=false;}
if(this.aliens.isDead()){this.won=true;this.playing=false;}}
if(this.dead||this.won){this.deathCount+=ig.system.tick;if(this.deathCount>=this.deathCountWait){this.reset();}}
if(this.debug==true)console.log("Currently: "+this.entities.length+" entities in existence.");},draw:function(){this.parent();var ctx=ig.system.context;ctx.beginPath();ctx.fillStyle="#FFF";ctx.font="20px Arial";ctx.fillText(ig.game.bullets+" Bullets",10,25);ctx.beginPath();ctx.strokeStyle="#FF0000";ctx.moveTo(0,this.height()-80);ctx.lineTo(this.width(),this.height()-80);ctx.stroke();ctx.beginPath();ctx.fillStyle="#FFF";ctx.font="14px Arial";ctx.fillText("Defeat",5,this.height()-85);ctx.beginPath();ctx.strokeStyle="#FFF";ctx.rect(this.width()-30,10,20,20);ctx.stroke();ctx.beginPath();ctx.strokeStyle="#FFF";ctx.moveTo(this.width()-25,15);ctx.lineTo(this.width()-15,25);ctx.stroke();ctx.beginPath();ctx.strokeStyle="#FFF";ctx.moveTo(this.width()-25,25);ctx.lineTo(this.width()-15,15);ctx.stroke();ctx.beginPath();ctx.strokeStyle="#FFF";ctx.rect(this.width()-60,10,20,20);ctx.stroke();if(this.playing==true){ctx.beginPath();ctx.strokeStyle="#FFF";ctx.moveTo(this.width()-55,17);ctx.lineTo(this.width()-45,17);ctx.stroke();ctx.beginPath();ctx.strokeStyle="#FFF";ctx.moveTo(this.width()-55,23);ctx.lineTo(this.width()-45,23);ctx.stroke();}
else{ctx.beginPath();ctx.strokeStyle="#FFF";ctx.moveTo(this.width()-54,15);ctx.lineTo(this.width()-45,20);ctx.lineTo(this.width()-54,25);ctx.lineTo(this.width()-54,15);ctx.stroke();}
if(this.dead){ctx.save();ctx.globalAlpha=0.8;ctx.beginPath();ctx.fillStyle="#B3B3B3";ctx.fillRect(0,0,this.width(),this.height());ctx.restore();ctx.beginPath();ctx.fillStyle="#FFF";ctx.font="70px Arial";ctx.fillText("You dead",(this.width()/2)-150,(this.height()/2)-40);}
if(this.won){ctx.save();ctx.globalAlpha=0.8;ctx.beginPath();ctx.fillStyle="#F7F700";ctx.fillRect(0,0,this.width(),this.height());ctx.restore();ctx.beginPath();ctx.fillStyle="#FFF";ctx.font="70px Arial";ctx.fillText("YOU WON",(this.width()/2)-170,(this.height()/2)-40);}}});ig.main("#game_screen",MyGame,60,434,510,1);});